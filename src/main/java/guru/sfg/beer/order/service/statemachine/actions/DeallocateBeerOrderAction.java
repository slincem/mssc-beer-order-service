...
public class DeallocateBeerOrderAction implements Action<BeerOrderStatusEnum, BeerOrderEventEnum> {
    ...
    public void execute(StateContext<BeerOrderStatusEnum, BeerOrderEventEnum> stateContext) {
        ...
        beerOrderRepository.findById(UUID.fromString(beerOrderId)).ifPresentOrElse(beerOrder -> {
            DeallocateBeerOrderRequest deallocationRequest = DeallocateBeerOrderRequest.builder()
                    .beerOrderDto(beerOrderMapper.beerOrderToDto(beerOrder))
                    .build();

            rabbitTemplate.convertAndSend(RabbitMQConfig.BEER_ORDER_EXCHANGE,
                    RabbitMQConfig.BEER_ORDER_DEALLOCATION_ROUTING_KEY,
                    deallocationRequest);

            log.debug("Sent Deallocation request to queue for order id: " + beerOrderId);
        }, () -> log.error("DeallocateBeerOrderAction. Beer Order Not Found. ID: " + beerOrderId));
    }
}
